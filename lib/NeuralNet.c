...
static Status NeuralNet_create_layer(NeuronLayer* l, u64 count) {
  ...
  dbg("NeuralNet_create_layer:+%p count=%" PRId64 "\n", (void*)l, count);
...
static Status Neuron_init(Neuron* n, NeuronLayer* inputs) {
  ...
  if (inputs == NULL) {
    ...
    dbg("Neuron_init:+%p inputs=%p count=%" PRId64 "\n", (void*)n, (void*)inputs, count);
    for (u64 w = 0; w < count; w++) {
      weights[w] = rand0_1() - 0.5;
      dbg("Neuron_init: %p weights[%" PRId64 "]=%lf\n", (void*)n, w, weights[w]);
    }
...
Status NeuralNet_init(NeuralNet* nn, u64 num_in_neurons, u64 num_hidden_layers,
    u64 num_out_neurons) {
  Status status;
  dbg("NeuralNet_init:+%p in_neurons=%" PRId64 " hidden_layers=%" PRId64 "" "out_neurons=%" PRId64 "\n",
...
static Status NeuralNet_add_hidden(NeuralNet* nn, u64 count) {
  ...
  dbg("NeuralNet_add_hidden:+%p count=%" PRId64 "\n", (void*)nn, count);
...
static Status NeuralNet_start(NeuralNet* nn) {
  ...
  dbg("NeuralNet_start: %p max_layers=%" PRId64 " last_hidden=%" PRId64 " out_layer=%" PRId64 "\n",
  ...
  for (u64 l = 0; l < nn->max_layers; l++) {
    ...
    dbg("NeuralNet_start: nn->layers[%" PRId64 "].count=%" PRId64 " in_layer=%p\n", l,
...
static u64 NeuralNet_get_points(NeuralNet* nn) {
  dbg("NeuralNet_get_points:+-%p count=%" PRId64 "\n", (void*)nn, nn->points);
  return nn->points;
}

static void NeuralNet_set_inputs(NeuralNet* nn, Pattern* input) {
  dbg("NeuralNet_set_inputs_:+%p count=%" PRId64 " input_layer count=%" PRId64 "\n",
...
static void NeuralNet_get_outputs(NeuralNet* nn, Pattern* output) {
  ...
  dbg("NeuralNet_outputs_:+%p count=%" PRId64 "\n", (void*)nn, output->count);
  ...
  for (u64 i = 0; i < count; i++) {
    ...
    dbg("NeuralNet_outputs_: %p output[%" PRId64 "]=%lf\n", (void*)nn, i, output->data[i]);
  }
...
static f64 NeuralNet_adjust_weights(NeuralNet* nn, Pattern* output,
    Pattern* target) {
  dbg("NeuralNet_adjust_weights_:+%p output count=%" PRId64 " target count=%" PRId64 "\n",
  ...
  for (u64 n = 0; n < output->count; n++) {
    ...
    dbg("NeuralNet_adjust_weights_: %" PRId64 ":%" PRId64 " err:%lf = target:%lf + output:%lf\n",
    ...
    dbg("NeuralNet_adjust_weights_: %" PRId64 ":%" PRId64 " pd_err:%lf ="
        " err:%lf * output[%" PRId64 "]:%lf * (1.0 - output[%" PRId64 "]:%lf\n",
    ...
    f64 sse = 0.5 * err * err;
    dbg("NeuralNet_adjust_weights_: %" PRId64 ":%" PRId64 " sse:%lf = 0.5 * err:%lf * err:%lf\n",
            nn->out_layer, n, sse, err, err);

    f64 tmp = nn->error;
    nn->error = tmp + sse;
    dbg("NeuralNet_adjust_weights_: %" PRId64 ":%" PRId64 " nn->error:%lf = nn->error:%lf + sse:%lf\n",
        nn->out_layer, n, nn->error, tmp, sse);
  }
  dbg("NeuralNet_adjust_weights_: out_layer:%" PRId64 " this.error=%lf\n",
  ...
  for (u64 l = nn->out_layer; l > first_hidden_layer; l--) {
    ...
    dbg("NeuralNet_adjust_weights_: %p cur_layer=%" PRId64 " prev_layer=%" PRId64 "\n", (void*)nn, l, l-1);
    ...
    for (u64 npl = 0; npl < prev_layer->count; npl++) {
      ...
      for (u64 ncl = 0; ncl < cur_layer->count; ncl++) {
        ...
        dbg("NeuralNet_adjust_weights_: %p cur_layer:%" PRId64 ":%" PRId64 " pd_err=%lf\n",
            (void*)nn, l, ncl, pd_err);
        f64 weight = cur_layer->neurons[ncl].weights[npl+1];
        dbg("NeuralNet_adjust_weights_: %p cur_layer:%" PRId64 ":%" PRId64 " weights[%" PRId64 "]=%lf\n",
            (void*)nn, l, ncl, npl+1, weight);
        sum_weighted_pd_err += pd_err * weight;
        dbg("NeuralNet_adjust_weights_: %p cur_layer:%" PRId64 ":%" PRId64 " sum_weighted_pd_err:%lf\n",
            (void*)nn, l, ncl, sum_weighted_pd_err);
      }

      f64 prev_out = prev_layer->neurons[npl].output;
      f64 pd_prev_out = prev_out * (1.0 - prev_out);
      dbg("NeuralNet_adjust_weights_: %p prev_layer:%" PRId64 ":%" PRId64 " pd_prev_out:%lf = "
      ...
      dbg("NeuralNet_adjust_weights_: %p prev_layer:%" PRId64 ":%" PRId64 " pd_error:%lf ="
  ...
  for (u64 l = 1; l <= nn->out_layer; l++) {
    NeuronLayer* layer = &nn->layers[l];
    dbg("NeuralNet_adjust_weights_: %p loop through layer %" PRId64 "\n", (void*)nn, l);
    for (u64 n = 0; n < layer->count; n++) {
      Neuron* neuron = &layer->neurons[n];
      ...
      dbg("NeuralNet_adjust_weights_: %p %" PRId64 ":%" PRId64 " momentums[%d]:%lf ="
      ...
      dbg("NeuralNet_adjust_weights_: %p %" PRId64 ":%" PRId64 " weights[%d]:%lf"
      ...
      dbg("NeuralNet_adjust_weights_: %p loop through neurons for %" PRId64 ":%" PRId64 ""
      ...
      for (u64 i = 0; i < neuron->inputs->count; i++) {
        ...
        dbg("NeuralNet_adjust_weights_: %p %" PRId64 ":%" PRId64 " momentums[%" PRId64 "]:%lf ="
        ...
        dbg("NeuralNet_adjust_weights_: %p %" PRId64 ":%" PRId64 " weights[%" PRId64 "]:%lf ="
            " weights[%" PRId64 "]:%lf + momentums[%" PRId64 "]=%lf\n",
